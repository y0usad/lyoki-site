...
export default function Checkout() {
    ...
    const shippingOptions = [
        { id: 'PAC2', name: 'PAC 2', price: 0, days: '3 - 7 dias úteis a partir da postagem' },
        { id: 'PAC_NORTE', name: 'PAC NORTE/NORDESTE', price: 55.00, days: '5 - 11 dias úteis a partir da postagem' },
        { id: 'SEDEX2', name: 'SEDEX 2', price: 55.00, days: '1 - 3 dias úteis a partir da postagem' },
        { id: 'SEDEX_NORTE', name: 'SEDEX NORTE/NORDESTE', price: 70.00, days: '2 - 6 dias úteis a partir da postagem' },
        { id: 'RETIRADA', name: 'Retirada no Estúdio', price: 0, days: 'Gratuito', location: 'Duque de Caxias, Duque de Caxias, Rio de Janeiro, 25223485, Brasil' }
    ]
        ...
        try {
            ...
            if (isAuthenticated) {
                const newOrder = {
                    ...
                    shippingMethod: shippingOptions.find(m => m.id === shippingMethod)?.name || '',
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2">
                        <div className="bg-white border-4 border-black p-8 shadow-[8px_8px_0px_0px_#DC143C]">
                            <h2 className="font-grunge text-3xl mb-2 text-black">CHECKOUT SEGURO</h2>
                            <Link to="/" className="text-sm text-gray-600 hover:text-lyoki-red underline mb-6 inline-block">
                                Continuar navegando
                            </Link>

                            {/* PayPal Express Checkout */}
                            <div className="mb-8 pb-8 border-b-2 border-gray-300">
                                <p className="text-center text-sm text-gray-600 mb-3 uppercase font-semibold">Checkout expresso</p>
                            ...
                            {!isAuthenticated && (
                                <div className="mb-6 bg-gray-50 p-4 border-2 border-gray-300">
                                    <p className="text-sm">
                                        Tem uma conta? <Link to="/login" className="text-lyoki-red font-bold hover:underline">Faça login</Link>
                                    </p>
                                </div>
                            )}
                            ...
                            {isAuthenticated && (
                                <div className="mb-6 bg-green-50 p-4 border-2 border-green-600">
                                    <p className="text-sm font-semibold text-green-800">
                                        ✓ Logado como {user?.email} | <Link to="/account" className="text-lyoki-red hover:underline">Ver perfil</Link>
                                    </p>
                                </div>
                            )}
                            ...
                            <form onSubmit={handlePayment} className="space-y-8">
                                ...
                                <div className="space-y-4">
                                    <h3 className="font-bold text-lg uppercase">Informações do cliente</h3>
                                ...
                                {/* Shipping Address */}
                                <div className="space-y-4">
                                    <h3 className="font-bold text-lg uppercase">Detalhes da entrega</h3>

                                    <div>
                                        <label className="block text-sm font-semibold mb-2">País/região *</label>
                                        <select
                                            name="country"
                                    ...
                                    <div>
                                        <label className="block text-sm font-semibold mb-2">Código postal (CEP) *</label>
                                        <input
                                            name="zipCode"
                                ...
                                {/* Shipping Method */}
                                <div className="space-y-4">
                                    <h3 className="font-bold text-lg uppercase">Método de entrega</h3>
                                    <div className="bg-gray-50 p-4 border-2 border-gray-300">
                                        <p className="text-sm font-semibold mb-3">Frete e entrega</p>
                                        <div className="space-y-2">
                                            {shippingOptions.map((option) => (
                                                <label key={option.id} className="flex items-start gap-3 p-3 bg-white border-2 border-gray-300 cursor-pointer hover:border-lyoki-red transition-colors">
                                                    <input
                                                        type="radio"
                                                        name="shipping"
                                                        value={option.id}
                                ...
                                {/* Billing Address */}
                                <div className="space-y-4">
                                    <h3 className="font-bold text-lg uppercase">Endereço de faturamento</h3>
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-gray-50 border-4 border-black p-6 shadow-[8px_8px_0px_0px_#DC143C] sticky top-24">
                            <div className="flex items-center justify-between mb-4">
                                <h2 className="font-grunge text-2xl text-black">RESUMO DO PEDIDO ({cart.length})</h2>
                                <Link to="/cart" className="text-sm text-lyoki-red hover:underline font-semibold">
                                    Editar carrinho
                                </Link>
                            </div>
                            ...
                            <div className="space-y-3 mb-6 max-h-[300px] overflow-y-auto pr-2">
                                {cart.map(item => (
                                    <div key={item.id} className="flex gap-3 items-center bg-white p-3 border-2 border-black">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h3 className="font-bold uppercase text-xs truncate">{item.name}</h3>
                                            <p className="text-gray-500 text-xs mt-1">Quant.: {item.quantity}</p>
                                        </div>
                                        <p className="font-bold text-sm flex-shrink-0">R$ {(item.price * item.quantity).toFixed(2)}</p>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="space-y-2 mb-6">
                                ...
                                >
                                    <Tag size={16} />
                                    <span className="font-semibold uppercase text-xs">Insira o código promocional</span>
                                </button>
                                <button
                                    type="button"
                                    className="w-full flex items-center gap-2 justify-center border-2 border-black p-3 hover:bg-black hover:text-white transition-colors text-sm"
                                >
                                    <FileText size={16} />
                                    <span className="font-semibold uppercase text-xs">Adicione uma observação</span>
                                </button>
                            </div>
                            ...
                            <div className="border-t-2 border-black pt-4 space-y-3">
                                ...
                                <div className="flex justify-between text-2xl font-bold pt-3 border-t-2 border-gray-300">
                                    ...
                                    <span className="text-lyoki-red">R$ {(total() + shippingCost).toFixed(2)}</span>
                                </div>
                            </div>

                            {/* Security Badge */}
                            <div className="mt-6 bg-black text-white p-3 text-center">
                                <p className="text-xs uppercase tracking-wider">🔒 Checkout seguro</p>
                            </div>
                        </div>
                    </div>
                </div>
